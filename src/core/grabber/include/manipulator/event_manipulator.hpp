...
public:
  ...
  void handle_keyboard_event(device_registry_entry_id device_registry_entry_id,
    ...
    // ----- start coder layout -------
    if (process_coder_layout(to_key_code, pressed, timestamp)) {
      return;
    }
    // ----- end coder layout -------
...
private:
  ...
  void post_key(std::string key_name, uint64_t timestamp) {
    post_key(key_name, true, timestamp);
    post_key(key_name, false, timestamp);
  }

  void post_key(std::string key_name, bool pressed, uint64_t timestamp) {
    if (auto key = krbn::types::get_key_code(key_name)) {
      if (!post_modifier_flag_event(*key, pressed, timestamp)) {
        post_key(*key, pressed, timestamp);
      }
    } else {
  ...
  bool process_key_in_layer(
    ...
    for (const auto& mapping : mappings) {
      if (key_code == mapping.from_key_code) {
        ...
        if (MOD_NAVIGATION && MOD_NAVIGATION_TAB_SHIFT && !pressed) {
          post_key("right_shift", false, timestamp);
        }
        return true;
      }
    }
  ...
  bool process_coder_layout(krbn::key_code key_code, bool pressed, uint64_t timestamp) {
    ...
    if (is_key(key_code, "left_shift")) {
      post_key("right_shift", pressed, timestamp);
      return true;
    }
    ...
    if (is_key(key_code, "non_us_backslash")) { post_key("right_shift", pressed, timestamp); return true; };
    if (is_key(key_code, "1") && !MOD_EDITING) { post_key("right_control", pressed, timestamp); return true; };
    if (is_key(key_code, "2") && !MOD_EDITING) { post_key("right_command", pressed, timestamp); return true; };
    if (is_key(key_code, "3") && !MOD_EDITING && !MOD_APP_SWITCH && !MOD_SPACEBAR) { post_key("right_option", pressed, timestamp); return true; };
    if (is_key(key_code, "9") && !MOD_EDITING) { post_key("right_option", pressed, timestamp); return true; };
    if (is_key(key_code, "0") && !MOD_EDITING) { post_key("right_command", pressed, timestamp); return true; };
    if (is_key(key_code, "hyphen") && !MOD_EDITING) { post_key("right_control", pressed, timestamp); return true; };
    if (is_key(key_code, "equal_sign")) { post_key("right_shift", pressed, timestamp); return true; };
    ...
    if (MOD_EDITING) {
      return process_key_in_layer(key_code, pressed, timestamp, coder_layout_.editing_mapping, true);
    }
    ...
    if (MOD_NUMBERS) {
      return process_key_in_layer(key_code, pressed, timestamp, coder_layout_.numbers_mapping, true);
    }
    ...
    if (is_key(key_code, "4") && !MOD_SPACEBAR) {
      ...
      if (pressed) {
        post_key("left_command", true, timestamp);
      } else {
        if (!app_switch_triggered) {
          post_key("tab", timestamp);
        }
        MOD_APP_SWITCH = false;
        app_switch_triggered = false;
        post_key("left_command", false, timestamp);
      }
      return true;
    }
    ...
    if (MOD_APP_SWITCH) {
      ...
      if (is_key(key_code, "3")) {
        if (pressed) {
          ...
          post_key("left_shift", true, timestamp);
          post_key("tab", timestamp);
          post_key("left_shift", false, timestamp);
        }
      }
    ...
    if (is_key(key_code, "3") && MOD_SPACEBAR) {
      if (pressed) {
        post_key("left_command", true, timestamp);
        post_key("left_control", true, timestamp);
        post_key("tab", timestamp);
        post_key("left_control", false, timestamp);
        post_key("left_command", false, timestamp);
      }
      return true;
    }
    ...
    if (pressed) {
      ...
      if (is_key(key_code, "spacebar")) {
        ...
        if (spacebar_timer_ != nullptr) {
          spacebar_timer_ = nullptr;
          if (MOD_NAVIGATION) {
            post_key("escape", timestamp);
          } else {
            post_key("spacebar", timestamp);
          }
          return true;
        } else {
          post_key("left_shift", false, timestamp);
    ...
    if (MOD_NAVIGATION) {
      if (MOD_SPACEBAR_SHIFT) post_key("left_shift", !pressed, timestamp); // de/activate shift from spacebar
      if (MOD_SPACEBAR) {
        return process_key_in_layer(key_code, pressed, timestamp, coder_layout_.navigation_extra_mapping, true);
      } else {
        return process_key_in_layer(key_code, pressed, timestamp, coder_layout_.navigation_mapping, true);
      }
    }
    ...
    if (MOD_CODING) {
      return process_key_in_layer(key_code, pressed, timestamp, coder_layout_.coding_mapping, false);
    }
